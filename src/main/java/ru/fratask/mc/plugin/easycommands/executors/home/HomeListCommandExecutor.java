...
public class HomeListCommandExecutor implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) {
        ...
        boolean haveHome = EasyCommandsPlugin.getInstance().getHomeTable().row((Player) src).size() > 0;
        if (!haveHome){
            src.sendMessage(Text.of(TextColors.RED, "You don't have home!"));
        } else {
            stringBuilder.append("Your homes [" + EasyCommandsPlugin.getInstance().getHomeTable().row((Player) src).size() + "]:");
            int counter = 1;
